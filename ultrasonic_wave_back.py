...
GPIO.setup(26, GPIO.OUT)  # Trig, 초음파 신호 전송핀 번호 지정 및 출력지정
GPIO.setup(19, GPIO.IN)  # Echo, 초음파 수신하는 수신 핀 번호 지정 및 입력지정
...
try:
    while True:
        ...
        GPIO.output(26, True)  # 10us 펄스를 내보냄
        time.sleep(0.00001)  # Python에서 이 펄스는 실제 100us 근처가 됨
        GPIO.output(26, False)  # 하지만 HC-SR04 센서는 이 오차를 받아줌

        while GPIO.input(19) == 0:  # echo 핀이 OFF 되는 시점을 시작 시간으로 잡음
             start = time.time()

        while GPIO.input(19) == 1:  # echo 핀이 다시 ON 되는 시점을 반사파 수신시간으로 잡음
            stop = time.time()
        ...
        if distance > 30:  # 거리가 15cm보다 크면 모터에게 ok, 그렇지 않으면 모터 stop
            msg = "ok"
        else:
            msg = "stop"
