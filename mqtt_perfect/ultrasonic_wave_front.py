...
GPIO.setup(6, GPIO.OUT)  # Trig, 초음파 신호 전송핀 번호 지정 및 출력지정
GPIO.setup(12, GPIO.IN)  # Echo, 초음파 수신하는 수신 핀 번호 지정 및 입력지정
...
try:
    while True:
        ...
        GPIO.output(6, True)  # 10us 펄스를 내보냄
        time.sleep(0.00001)  # Python에서 이 펄스는 실제 100us 근처
        GPIO.output(6, False)  # 하지만 HC-SR04 센서는 이 오차를 받아줌

        while GPIO.input(12) == 0: # echo 핀이 OFF 되는 시점을 시작 시간으로 잡음
             start = time.time()

        while GPIO.input(12) == 1: # echo 핀이 다시 ON 되는 시점을 반사파 수신시간으로 잡음
            stop = time.time()
        ...
        time.sleep(1)
        print ('Front Distance => ', distance, 'cm')  # 실제 거리 출력
        if distance < 30:  # 30cm 보다 가까우면 모터에게 back, 60cm 보다 멀면 모터에게 go, 그 사이이면 ok
            msg = "back"
        elif distance > 60:
            msg = "go"
        else:
